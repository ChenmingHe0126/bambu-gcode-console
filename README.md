...
- **G-code 文件**:拖入 .gcode 文件 → 浏览器内渲染 XY 刀路预览(挤出/空移分色、层滑块、支持 G2/G3 圆弧),然后二选一:
  - **Stream**:逐行代发(Printrun 式),可暂停/停止,进度条 + 预览图上高亮已执行部分;适合几百行的课堂演示文件
  - **Upload to SD**(默认拖入即自动上传):打包成 Bambu Studio 风格的完整 `.gcode.3mf`(含刀路缩略图、A1 机型元数据、护栏注释、md5),FTPS 传进 SD 卡 —— 然后**在打印机屏幕上选中它开始打印**。SD 文件列表支持刷新/删除
- 状态栏实时显示打印机状态和 WiFi 信号

> 注:远程一键启动 SD 打印(MQTT `project_file`)在 2026 年的 A1 固件上会被"受理但不执行",目前以打印机屏幕启动为准;Print 按钮保留作实验用途。
